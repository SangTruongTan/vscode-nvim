...
- **Paste Without Overwriting**: Use `P` (capital) in visual mode to paste without losing your yanked content.
- **Replace Word**: `viwp` (Visual Inner Word Paste) - quickly override word under cursor.
- **Yank Rest of Line**: `y$` (Standard command).
- **Yank Whole Line (No Newline)**:
  - `0y$`: Start at absolute beginning.
  - `^y$`: Start at first non-whitespace char.
